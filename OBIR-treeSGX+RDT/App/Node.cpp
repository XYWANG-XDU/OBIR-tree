...
void Node::addVirtualBranch(map<int,string> text_position_map,int temp_ID,string Com)
{
    for(int index = int(this->mBranch.size()); index < MAX_SIZE; index++)
    {
        ...
        int virtual_ID = -1;
        char tmp_line[200];
        s= generate_random_block(0,B - 16 - sizeof(uint32_t),tmp_line);
        string vid_str = string((const char*)(&virtual_ID), sizeof(uint32_t));
        string cipher;
        string da=tmp_line + vid_str;
        const char* tem =da.data();
        s=aes_encrypt(0,tem,tmp_line);
